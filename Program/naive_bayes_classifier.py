...
class NaiveBayesClassifier:
    ...
    def classify(self):       
        ...
        for i in self.tweets:
            ...
            for t in tw:
                ...
                # print "neg_words : %s, pos_words : %s" % (neg_words, pos_words)
                score = math.log(len(pos_words)+0.5) - math.log(len(neg_words)+0.5)
                # print "%s | %s, score : %f" % (label, t, score)

                # print "t : %s, label : %s" % (t, label)
                # dist = self.classifier.prob_classify(self.helper.extract_features(t.split()))
    ...
    def accuracy(self):
        ...
        for (t, l) in tweets:
            ...
            if(label == l):
                correct+= 1
                # print 'Correct = %d, label : %s' %(correct, label)
            else:
                wrong+= 1
                # print 'Wrong = %d, label : %s' %(wrong, label)
            total += 1
        #end loop
